...
capture <- function(
  ...
  for (i.langue in c('fr', 'en', 'bil')) {
    ...
    temp <- graph_capture_totale2(
      ...
      nbPecheursMoy.strate = nb_pecheurs_tot_par_strate,
      langue = i.langue
    )
    dev.off()
  }
  ...
  for (i.langue in c('fr', 'en', 'bil')) {
    ...
    par(mfrow = c(1, 2))
    temp <- graph_capture_prop_esp(capt = captTot.site.sfs, langue = i.langue)
    temp <- graph_capture_prop_esp2(
