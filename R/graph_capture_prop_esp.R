...
#'
graph_capture_prop_esp <- function(
  capt,
  titre = 'A',
  langue = c('fr', 'en', 'bil')
) {
  ...
  x <- barplot(
    ...
    col = c(2, 3, 4, 5, 6),
    xlab = xlab1,
    ylab = ylab1 #, legend=TRUE,
    ## args.legend=list(x='topright',inset=0.03,bg="white",title=legende1,
    ##                  legend=rev(c(espSeb,espMor,espOgac,paste0(espMor,'+',espOgac),espTur)),
    ##                  fill=rev(c(2,3,4,0,5)))
  )
