#' Gaphiques de la standardisation des NUE récoltées par les échantillonneurs, pour l'ensemble du Saguenay
#'
...
#' @param annees vector() des années disponibles
...
graph_std_nue_ech <- function(
  ...
  if (FALSE) {
    ...
    i.esp <- 1
    dat.esp <- subset(dat, anneeGestion %in% annee.espece[[i.esp]])
  ...
  for (i.esp in seq_along(especes)) {
    data.secteur <- subset(
      dat,
      anneeGestion %in% annee.espece[[especes[i.esp]]]
    )
    coeffGraph <- list()
    ...
    dimnames(temp) <- list(
      c('valeur', 'IC95-', 'IC95+'),
      annee.espece[[especes[i.esp]]]
    )
    val[[especes[i.esp]]] <- temp
    temp <- cbind(
      annee = annee.espece[[especes[i.esp]]],
      exp(cg$annee$coeff + cg$annee$intercept)
    )
    temp <- merge(temp, list(annee = annees), all = TRUE, by = 'annee')
    plot(
      temp,
      type = "o",
      ylim = c(
    ...
    for (i.an in seq_along(cg$anneeGestion$coeff)) {
      lines(
        rep(annee.espece[[especes[i.esp]]][i.an], 2),
        exp(c(
